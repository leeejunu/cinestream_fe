            ...
            <section className="mb-16">
              <div className="flex flex-wrap items-center justify-between mb-8 gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className={`text-2xl font-bold transition-colors ${isDark ? "text-white" : "text-slate-900"}`}>이번 주 인기 급상승</h2>
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className={`ml-2 w-8 h-8 rounded-full transition-colors ${isDark ? "bg-slate-800 text-white hover:bg-slate-700" : "bg-slate-200 text-slate-900 hover:bg-slate-300"}`}
                ...
                <div className="flex items-center gap-2 hidden sm:flex">
                  <Button 
                    variant="outline" 
                    size="icon" 
                    className={`rounded-full transition-colors ${isDark ? "bg-slate-900 border-slate-700 text-white hover:bg-slate-800" : "bg-white border-slate-200 text-slate-900 hover:bg-slate-50"}`}
                    onClick={() => scroll("left")}
                  >
              ...
              <div ref={scrollRef} className="flex gap-6 overflow-x-auto snap-x scrollbar-hide pb-6 px-2 -mx-2">
                {[...allMovies]
                  .sort((a, b) => {
                  ...
                  <Card
                    key={movie.movieId}
                    className={`min-w-[220px] max-w-[220px] md:min-w-[280px] md:max-w-[280px] shrink-0 snap-center cursor-pointer hover:border-purple-500 hover:-translate-y-2 transition-all duration-300 overflow-hidden group shadow-lg ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
                  ...
                  >
                    <div className="aspect-[2/3] relative overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <PlayCircle className="w-16 h-16 text-white drop-shadow-lg scale-75 group-hover:scale-100 transition-transform duration-300" />
                      </div>
                      {movie.averageRating !== null && (
                        <div className="absolute top-3 right-3">
                          <Badge className="bg-slate-900/80 backdrop-blur-md text-amber-400 border border-slate-700 flex items-center gap-1 shadow-md">
                            <Star className="w-3 h-3 fill-amber-400" /> {movie.averageRating.toFixed(1)}
                          </Badge>
                        </div>
                      )}
                    </div>
                    <CardContent className={`p-5 transition-colors ${isDark ? "bg-gradient-to-b from-slate-900 to-slate-950" : "bg-white"}`}>
                      <h3 className={`font-bold text-lg mb-1 truncate transition-colors group-hover:text-purple-500 ${isDark ? "text-white" : "text-slate-900"}`}>{movie.title}</h3>
                      <div className="flex items-center justify-between mt-3">
                        <p
                          className={`text-sm transition-colors user-select-none ${isDark ? "text-slate-400 hover:text-white" : "text-slate-500 hover:text-slate-900"}`}
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/creator/${movie.creatorId}`);
                          }}
                        >
                          {movie.nickname}
                        </p>
          ...
          <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                <h2 className={`text-3xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}>검색 결과</h2>
                {!searchLoading && (
            ...
            {searchLoading ? (
              ...
              <div className="text-center py-24 px-6 rounded-3xl border-2 border-dashed border-slate-800/50 bg-slate-900/20 backdrop-blur-sm">
                ...
                <p className="text-slate-500 max-w-sm mx-auto">
                  다른 검색어를 입력하시거나 태그 필터를 변경하여 다시 시도해 보세요.
                </p>
                <Button
                  onClick={() => {
                    setSearchQuery("");
                  ...
                  variant="outline"
                  className="mt-8 border-purple-500/30 text-purple-500 hover:bg-purple-500/10"
                >
                  검색 초기화
                </Button>
              </div>
